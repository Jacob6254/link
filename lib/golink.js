...
import { ESCAPE_FN } from "@/lib/escape";
...
(function () {
  ...
  // — c'est la seule façon fiable quand la page s'est chargée sans geste.
  if (canEscape) {
  ...
  document.getElementById("open").addEventListener("click", function(){
    if (canEscape) amsEscape(web, web); else location.href = web;
  });
  ...
  function step2(){
    if (left) return;
    if (canEscape) {
      amsEscape(web, null);   // souvent bloqué sans geste : d'où le tap ci-dessus
      setTimeout(showManual, 900);
    } else {
