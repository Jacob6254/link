...
import { ANIMATIONS, BRAND_ICONS, FONTS, HEADER_STYLES, PRESETS } from "@/lib/pagerender";
...
export function sanitizeButtonExtras(body, patch) {
  ...
  if (body.icon !== undefined) {
    ...
    if (icon.startsWith("brand:") && !BRAND_ICONS[icon.slice(6)]) {
      return "Unknown brand icon";
    }
  ...
  if (body.font !== undefined) {
    patch.font = FONTS[body.font] ? body.font : null;
  }
  return null;
}
