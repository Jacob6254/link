...
export const BRAND_ICONS = {
  onlyfans: {
    name: "OnlyFans",
    bg: "#ffffff",
    // Deux cercles concentriques : bleu à l'extérieur, blanc au centre. Un
    // tracé simple reste net à 17px comme à 27px, là où le logo complet
    // (anneau + virgule) devient illisible.
    svg: `<svg viewBox="0 0 24 24" width="100%" height="100%"><circle cx="12" cy="12" r="11.5" fill="#00AEEF"/><circle cx="12" cy="12" r="4.2" fill="#fff"/></svg>`,
  },
  instagram: {
    name: "Instagram",
    bg: "#ffffff",
...
export function resolveTheme(theme = {}) {
  ...
  t.badgeColor = /^#[0-9a-fA-F]{6}$/.test(theme.badgeColor || "")
    ? theme.badgeColor
    : "#3b9eff";
  t.flatBg = !!theme.flatBg;
  t.btnSize = ["sm", "md", "lg"].includes(theme.btnSize) ? theme.btnSize : "md";
  t.btnBorderWidth = clampInt(theme.btnBorderWidth, 0, 8, 0);
  t.btnBorderColor = /^#[0-9a-fA-F]{6}$/.test(theme.btnBorderColor || "")
    ? theme.btnBorderColor
    : "#ffffff";
  t.btnRadius = clampInt(theme.btnRadius, 0, 40, -1);
  t.btnImageHeight = clampInt(theme.btnImageHeight, 70, 300, 96);
...
const BTN_SIZES = {
  sm: { pad: "11px 16px", font: "0.9rem" },
  md: { pad: "16px 20px", font: "0.98rem" },
  lg: { pad: "22px 24px", font: "1.12rem" },
};

// Rayon effectif : la valeur libre (btnRadius) prime sur la forme prédéfinie.
function buttonRadius(t) {
  return t.btnRadius >= 0 ? `${t.btnRadius}px` : SHAPES[t.btnShape] || SHAPES.rounded;
}

function buttonCss(t) {
  const radius = buttonRadius(t);
  const size = BTN_SIZES[t.btnSize] || BTN_SIZES.md;
  const base = `padding:${size.pad};font-size:${size.font};border-radius:${radius};`;
  // Une bordure explicite l'emporte sur celle du style choisi.
  const border = t.btnBorderWidth
    ? `border:${t.btnBorderWidth}px solid ${t.btnBorderColor};`
    : null;

  if (t.btnFill === "outline") {
    return `${base}background:transparent;color:${t.text};${border || `border:2px solid ${t.accent};`}`;
  }
  if (t.btnFill === "glass") {
    return `${base}background:rgba(255,255,255,0.10);color:${t.text};${border || "border:1px solid rgba(255,255,255,0.22);"}backdrop-filter:blur(8px);`;
  }
  return `${base}background:${t.accent};color:${contrastText(t.accent)};${border || "border:none;"}`;
}
...
function buttonHtml(b, { preview, visitId }) {
  const anim =
    ANIMATIONS[b.animation] && b.animation !== "none" ? ` anim-${b.animation}` : "";

  // Un bloc "heading" n'est pas un lien : c'est un intertitre entre les boutons.
  if (b.kind === "heading") {
    const font = FONTS[b.font];
    const style = font && font.css ? ` style="font-family:${font.css}"` : "";
    return `<p class="btn-heading${anim}"${style}>${esc(b.label)}</p>`;
  }
...
export function renderPageHTML(page, buttons, { preview = false, visitId = null } = {}) {
  ...
  // Polices à charger : celle de la page + celles choisies sur les intertitres.
  const googleFonts = [
    ...new Set(
      [font, ...(buttons || []).map((b) => FONTS[b.font])]
        .filter((f) => f && f.gf)
        .map((f) => f.gf)
    ),
  ];
  const btns = (buttons || [])
    ...
    .join("\n      ");
  // Les intertitres ne comptent pas : c'est le nombre de liens qui décide.
  const solo = (buttons || []).filter((b) => b.kind !== "heading").length === 1;
...
${googleFonts.length ? `<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
...
body{
  min-height:100dvh;
  background:${t.flatBg ? t.bg2 : `linear-gradient(160deg,${t.bg1},${t.bg2})`};
  color:${t.text};
...
.hero-shade{
  ...
  background:linear-gradient(transparent 35%,rgba(0,0,0,0.55) 72%,${t.flatBg ? t.bg2 : t.bg1});
}
...
/* Un seul bouton : il est perdu en haut d'une page presque vide. Les marges
   auto le recentrent dans l'espace libre ; s'il n'y en a pas, elles valent 0
   et la mise en page ne bouge pas. */
.btns.solo{margin-top:auto;margin-bottom:auto;padding:32px 0}
.btn{
...
.btn-img{
  position:relative;min-height:${t.btnImageHeight}px;padding:0;overflow:hidden;
  background-size:cover;background-position:center;
...
/* Le libellé, pas la pastille d'icône : celle-ci doit rester en absolu. */
.btn-img span:not(.btn-icon){
...
.btn-icon img{width:100%;height:100%;object-fit:cover;display:block}
.btn-icon svg{width:27px;height:27px;flex:0 0 auto;display:block}
.btn:not(.btn-img) .btn-icon{position:static;width:24px;height:24px;font-size:0.85rem;margin-right:8px;box-shadow:none}
.btn:not(.btn-img) .btn-icon svg{width:17px;height:17px}
...
/* Entrée en cascade — seulement pour les blocs sans animation choisie,
   sinon la propriété "animation" écraserait celle du propriétaire. */
.btns > *:not([class*="anim-"]){animation:btnIn 0.5s cubic-bezier(0.22,0.9,0.3,1) both}
...
/* Les animations d'apparition sont décoratives : on les coupe si le système
   demande moins de mouvement. En revanche les animations .anim-* sont un
   réglage explicite du propriétaire de la page, on les conserve. */
@media (prefers-reduced-motion:reduce){
  .btn{transition:none}
  .btns > *:not([class*="anim-"]),.avatar,.hero{animation:none}
}
</style>
</head><body>
  <main class="wrap">
