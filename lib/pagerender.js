...
// Sortie du navigateur intégré (Instagram, TikTok…) sur iOS.
// CRUCIAL : le changement de schéma doit se faire DANS le handler du clic,
// tant que le "user gesture" est encore valide — sinon WKWebView le bloque.
// Le tap ouvre Safari sur notre propre /b/<id>, qui fait ensuite le deep-link
// et le tracking depuis un vrai navigateur.
const ESCAPE_SCRIPT = `<script>
(function () {
  var ua = navigator.userAgent || "";
  var isIOS = /iPhone|iPad|iPod/i.test(ua) ||
    (navigator.platform === "MacIntel" && navigator.maxTouchPoints > 1);
  var inApp = /Instagram|FBAN|FBAV|FB_IAB|FBIOS|Messenger|TikTok|BytedanceWebview|musical_ly|Trill|Snapchat|LinkedInApp|Pinterest|Line\\/|KAKAOTALK/i.test(ua);
  ...
  document.addEventListener("click", function (e) {
    ...
    e.preventDefault();
    var full = location.protocol + "//" + location.host + href;
    var left = false;
    ...
    // Si Safari ne s'ouvre pas, on suit le lien normalement.
    setTimeout(function () { if (!left) location.href = href; }, 1100);

    // Synchrone, dans le geste : c'est ce qui rend l'ouverture possible.
    location.href = "x-safari-" + full;
  }, true);
})();
</script>`;
